...
type ChannelOtherSettings struct {
	...
	// TaskApiPath overrides the path suffix (from the channel Base URL) that the
	// doubao video task adaptor uses for task create/query endpoints. Leave empty
	// to use the built-in `/api/v3/contents/generations/tasks` (volcengine ark
	// gateway). Set e.g. `/api/v1/contents/generations/tasks` to target gateways
	// that re-host the same Seedance protocol under a different path.
	TaskApiPath string `json:"task_api_path,omitempty"`
	// AssetGroupApiPath overrides the path suffix (from the channel Base URL) that
	// the asset-group adaptor uses. Leave empty to use the built-in
	// `/api/v1/asset-groups`. Set e.g. `/api/v1/contents/management/assets-groups`
	// to target gateways that re-host the same asset-group protocol under a
	// different path.
	AssetGroupApiPath string `json:"asset_group_api_path,omitempty"`
	// AssetApiPath overrides the path suffix (from the channel Base URL) that the
